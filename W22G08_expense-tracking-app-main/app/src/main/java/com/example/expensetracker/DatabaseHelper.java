...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public boolean editData(TransactionModel transactionModel) {
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues cv = new ContentValues();

        // Thêm các trường cần cập nhật
        cv.put(COLUMN_DATE, dateFormat.format(transactionModel.getDate())); // Ngày (định dạng yyyy-MM-dd)
        cv.put(COLUMN_CATEGORY, transactionModel.getCategory()); // Danh mục
        cv.put(COLUMN_NOTE, transactionModel.getNote()); // Ghi chú
        cv.put(COLUMN_GROUP, transactionModel.getGroup()); // Nhóm
        cv.put(COLUMN_AMOUNT, transactionModel.getAmount()); // Số tiền

        // Điều kiện để xác định mục cần sửa
        String whereClause = COLUMN_ID + " = ?";
        String[] whereArgs = {String.valueOf(transactionModel.getId())};

        // Thực hiện cập nhật
        int result = db.update(EXPENSE_TABLE, cv, whereClause, whereArgs);
